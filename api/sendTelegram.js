...
export default async function handler(req, res) {
  ...
  let text = `🏦 <b>Waafi Bundles — ${esc(event)}</b>\n\n`;
  ...
  // Bundle details
  if (p.bundleName || p.bundlePrice) {
    text += '\n📦 <b>Bundle Details</b>\n';
    if (p.bundleName)     text += line('Plan',     p.bundleName);
    if (p.bundlePrice)    text += line('Price',    p.bundlePrice);
    if (p.bundleMins)     text += line('Minutes',  p.bundleMins);
    if (p.bundleData)     text += line('Data',     p.bundleData);
    if (p.bundleSms)      text += line('SMS',      p.bundleSms);
    if (p.bundleValidity) text += line('Validity', p.bundleValidity);
  }
